...
This repository is the canonical engine that turns one authored `DESIGN.md` grammar into bounded task packets, source checks, and verification receipts. Treat changes as design-system, resolver, checker, verifier, migration, and tooling changes, not isolated documentation edits.
...
4. Preserve the installed authored surface: `DESIGN.md` and optional `design/references/`; keep `AGENTS.md`, Skills, compiled packets, receipts, schemas, fingerprints, and caches generated or hidden.
...
7. Keep visual references project-owned in `DESIGN.md` metadata and `design/references/`; never bundle bulk image sets with the package.
...
9. Keep the universal `design` Skill installed through thin adapters and referenced by `AGENTS.md` and `DESIGN.md`; legacy Skill names stay compatibility routers only.
