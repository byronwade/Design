...
`design verify --mode release` must produce a receipt and evidence manifest that covers rendered surface structure, accessibility semantics, keyboard and focus behavior, responsive coverage, overflow coverage, resolved state-matrix coverage, browser screenshot or deterministic static-capture output, and baseline comparison when an approved baseline manifest exists. Missing categories, missing resolved states, failed error-level source checks, stale context, expired exceptions, and unapproved baseline changes block release verification.
