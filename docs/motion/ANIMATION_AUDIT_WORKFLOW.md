...
The rule catalog with precise values lives in the project skill's `AUDIT.md`. The plan format lives in `PLAN-TEMPLATE.md`. Load them when you audit and when you write plans. Also read the repository root `DESIGN.md`, `docs/DESIGN_SYSTEM.md`, and `docs/motion/APPLE_FLUID_INTERACTION.md` when available.
...
Audit against eight categories:
...
After the table, list 2–4 **missed opportunities** — places that don't animate but should — separately, since they're additive rather than corrective.
...
One plan per selected finding, written into `plans/` as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).

Write for the weakest executor: exact file paths and current-code excerpts, exact target values (cubic-beziers, durations, spring configs), the repo's own conventions with an exemplar, ordered steps, hard scope boundaries, and a verification section including how to feel-check the result (slow motion, frame-by-frame, real device for gestures).
...
| `execute <plan>` | Dispatch an executor subagent to implement the plan in an isolated worktree, then review its diff |
| `reconcile` | Re-check `plans/` against current code: mark done plans DONE, refresh stale references, retire fixed findings |

## Tone

State findings plainly with evidence. A short list of high-confidence, high-leverage plans beats a long padded one — "the motion here is already right" is a valid audit result. Flag uncertainty honestly: when feel can't be judged from code alone, say so and put a feel-check step in the plan instead of guessing.
